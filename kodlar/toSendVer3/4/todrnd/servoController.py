# pigpio servoları için lazım
import pigpio
from numpy import interp
import time

# SERVO PİNLERİ
# panning ve tilting servoları
panServo = 4
tiltServo = 17

# bilek ve dirsek pinleri
wristServo = 10
elbowServo = 11
...
# yarım açık olsunlar
# dirsek ve bilek açılar
elbowPos = 750
wristPos = 750

# panning ve tilting açıları
idle_panPos = 1500
idle_tiltPos = 1500

# servo sisteminin tanımlanması ve başlatılması
servo = pigpio.pi()
...
# panning ve tilting için çalışacak olan fonksiyon
def panAndTilt(x, y, w, h) -> int:
   ...
   # eğer int(x+(w/2)) > 360 ise kırmızı nokta resim karesinin sağ tarafından demektir
    if int(x+(w/2)) > 360:
        panPos = int(panPos - interp(int(x+(w/2)), (360, 640), (minMov, maxMov)))

    # eğer int(x+(w/2)) < 280 ise kırmızı nokta resim karesini sol tarafından demektir
    elif int(x+(w/2)) < 280:
        panPos = int(panPos + interp(int(x+(w/2)), (280, 0), (minMov, maxMov)))

    # eğer int(y+(h/2)) > 280 ise kırmızı nokta resim karesinin alt tarafından demektir.
    if int(y+(h/2)) > 280:
        tiltPos = int(tiltPos + interp(int(y+(h/2)), (280, 480), (minMov, maxMov)))

    # eğer int(y+(h/2)) < 200 ise kırmızı nokta resim karesinin üst tarafında demektir.
    elif int(y+(h/2)) < 200:
        tiltPos = int(tiltPos - interp(int(y+(h/2)), (200, 0), (minMov, maxMov)))

    # kırmızı nokta ortalandı mı kontrol et
    if  is_idle(idle_tiltPos, tiltPos, idle_panPos, panPos):

        # eğer ortalama başarılıysa bilek ve dirsek için dönecek olan değeri hesaplamaya başla
...
# dirsek ve bilek için çalışacak olan fonksiyon
...
# cismin ekranın tam ortasından olup olmadığını
